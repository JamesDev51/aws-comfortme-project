...
def lambda_handler(event, context):
    try:
        print("--------------------------------")
        print("getCheerUpCount lambda_handler function init")
        params=event['queryStringParameters']
        consolePostId=params['consolePostId']
        print("consolePostId : ",consolePostId)
        
        sql = f"SELECT COUNT(*) AS 'cheerUpCount' FROM CheerUp WHERE consolePostId=%s"
        ...
        print("getCheerUpCount lambda_handler function done")
        print("--------------------------------")
        return result
    except Exception:
